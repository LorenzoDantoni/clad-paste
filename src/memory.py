...
class Memory():
    ...
    def get_samples_from_memory(self, memory_batch_size):
        ...
        random_task_ids = np.random.choice(tasks,size = memory_batch_size,replace=True)#if you have current_task=3 then tasks=[0,1,2], 
        #and if batch_size=8 then random_tasks_ids=[22112002]
    ...
    def create_batch_data(self, batch_data, memory_batch_size):#combines batch from memory(previous tasks) and batch from current task
        '''
        ...
        if self.strategy.index_training>0 and self.strategy.sample_strategy not in ["naive","cumulative","multi_task","single_model"]:
            ...
            if self.strategy.parameters['architecture'] == "draem":
                ...
                batch_old_samples_0 = torch.cat([batch_old_samples[0], batch_data[0]])#size: 2x memory batch size
...
class MemoryFromFolder(Memory):
    ...
    def create_memory_for_each_task(self):
        ...
        for task_id,class_id in enumerate(class_ids):#for each class taken prior to current task it creates tasks_memory[task_id] by loading stored samples
            dir_path = os.path.join(self.dir_experiment_path, f"{self.folder_name}", f"T{self.current_task-1}", f"{class_id}").replace('\\','/')#/data/original_dataset/memorized/0/class_id0
