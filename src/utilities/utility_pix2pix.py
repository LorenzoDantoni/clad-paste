...
def forward_test(model, real_A, real_B, device):
    model.real_A = real_A.to(model.device)#blurred 3x224x224
    if real_B!=None:
        model.real_B = real_B.to(model.device)#original 3x224x224
    model.test()
    fake_B = model.fake_B
    return fake_B
...
def logging_images(self,batch, index_sample_batch,real_A, real_B, fake_B, dataloader, complete_dataset, index_training, batch_index):
    ...
    dir_path = os.path.join( self.strategy.path_logs, f"{self.mode}", f"T{index_training}",  f"{class_id}" ).replace('\\','/')
    ...
    if self.mode=="test" or self.mode=="train":
        ...
        filepath =  os.path.join(dir_path, f"summary_{sample_id}.png" ).replace('\\','/')
        summary.save(filepath)

        img = convert2img(fake_B[0])
        img = Image.fromarray(img) 
        filepath_img = os.path.join(dir_path,f"{sample_id}.png").replace('\\','/')
    ...
    if "reconstruct" in self.mode or self.mode=="evaluate_data":
        ...
        if batch_index==(len(dataloader)-1) and index_sample_batch==0:  
           #self.strategy.run[f"Task_Results/T{index_training}"][f"visual_{mode}"].log(summary) 
            self.strategy.run.log({f"Task_Results/T{index_training}/visual_{mode}": wandb.Image(summary)})
        filepath_diz = os.path.join(dir_path,f"{sample_id}.pickle").replace('\\','/')
...
def produce_reconstruction_inpaint(trainer,model, complete_dataset, batch, index_sample_batch):
    ...
    for k in k_values:
        ...
        dir_path = os.path.join( strategy.path_logs, f"T{index_training}", "visual_val" ).replace('\\','/')
        os.makedirs(dir_path, exist_ok=True)
        filepath =  os.path.join(dir_path, f"{index_dataset}_k={k}.png" ).replace('\\','/')
        summary.save(filepath)
...
def produce_scaled_A_from_B(img,h,w):#3x224x224 tensor Normalized,32,32
    img = img.detach().cpu().numpy()
    img_size = img.shape[1]
    img = convert2img(img)

    img = resize_numpy_img(img,h)
    img_32 = img.copy()
    img = resize_numpy_img(img,img_size)

    transform_tensor = create_transform_img(img_size, img_size)
    img = Image.fromarray(img) 
    img = transform_tensor(img)

    return img, img_32#tensor3x224x224,img