...
def init_strategy_variables(strategy, complete_train_dataset,complete_test_dataset,train_stream,test_stream,original_complete_train_dataset, original_complete_test_dataset, original_train_stream, original_test_stream,labels_map,run,path_logs,elapsed_time):
...
def init_execute(credentials_path, default_path,parameters_path, seed, create_run=True) -> tuple:
    ...
    Args:
        ...
        default_path (str) : path to the default (common) parameters json path
        parameters_path (str) : path to the model and method parameters json path
        seed (int): seed that must be used in the simulation
    ...
    f = open(f"configurations/models/{parameters['architecture']}.json", "rb")#models/pix2pix.json???
    parameters_architecture = json.load(f)#model characteristic
    parameters_architecture_copy = parameters_architecture.copy()
    f.close()

    #define the final parameters for the model
    new_parameters = {}
    ...
    #added first time for DRAEM purpose
    ...
    #change seed when rerunning the program in order to obtain different results
    random.seed(seed)
    ...
    #    important params to be tracked
    #    parameters.setdefault("memory_dataset_path_train", "")
    #    parameters.setdefault("type_memory_train","memorized")
    #    parameters.setdefault("memory_reconstruct_dataset_path_train", "")
    #    parameters.setdefault("type_memory_reconstruct_train","")
    ...
    #    anomaly_detection_task = parameters.setdefault("anomaly_detection_task", False)
    #    anomaly_detection_task_with_metrics = parameters.setdefault("anomaly_detection_task_with_metrics", True)
    wandb.login(key=credentials["api_token"])
    ...
    if create_run:
        if "run_name" not in parameters:
            ...
            run = wandb.init(project=credentials["project_name"],
                             entity = credentials["entity"],
                tags = parameters["tags"],
                run=run_name )
    else:
...
def load_and_split_dataset(parameters,dataset_name,num_tasks,task_order):
    ...
    cl_benchmark = ContinualLearningBenchmark(complete_train_dataset, complete_test_dataset, num_tasks, task_order)#this reorganize already loaded train and test datasets in desired task order
...
def manage_dataset(strategy, parameters,complete_train_dataset,complete_test_dataset,train_stream,test_stream):
    ...
    dataset_path_train = strategy.parameters.get("dataset_path_train")#"/data/original_dataset"
    dataset_path_test = strategy.parameters.get("dataset_path_test")#"/data/original_dataset"
    type_folder_train = strategy.parameters.get("type_folder_train") #returns "memorized" for example for pix2pix
    type_folder_test = strategy.parameters.get("type_folder_test") #"evaluate_data" for example for pix2pix
    #neglect the comments, everything is empty!!! ""
    task_order = strategy.task_order
    ...
    if dataset_path_test!="" and type_folder_test!="":
        ...
        for test_dataset in test_stream:
            for i in range(len(test_dataset)):
                index_dataset = test_dataset[i][2]#idx of the image from test_dataset
    ...
    if new_memory:
        for task_id_old in range(0,self.index_training+1):
            ...
            if self.index_training==task_id_old:
                ...
                # self.memory.memory_update(self.task_train_dataset, self.index_training, task_id_old)
            else:
                ...
                # self.memory.memory_update(self.memory.tasks_memory[task_id_old], self.index_training, task_id_old)
